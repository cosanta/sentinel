...
def get_dash_conf():
    if sys.platform == 'win32':
        dash_conf = os.path.join(os.getenv('APPDATA'), "cosanta/cosanta.conf")
    else:
        home = os.environ.get('HOME')

        dash_conf = os.path.join(home, ".cosanta/cosanta.conf")
        if sys.platform == 'darwin':
            dash_conf = os.path.join(home, "Library/Application Support/Cosanta/cosanta.conf")
