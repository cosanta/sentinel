...
if [ ! -f /sentinel/cosanta.conf ]; then
  if [ -z "$RPCUSER" -o -z "$RPCPASSWORD" -o -z "$RPCPORT" ]; then
    echo "When no /sentinel/cosanta.conf is present, you must at least set RPCUSER, RPCPORT and RPCPASSWORD environment variables"
    exit 1
  fi

  echo "" > /sentinel/cosanta.conf
  if [ -n "$RPCUSER" ]; then
    echo "rpcuser=${RPCUSER}" >> /sentinel/cosanta.conf
  fi
  if [ -n "$RPCPASSWORD" ]; then
    echo "rpcpassword=${RPCPASSWORD}" >> /sentinel/cosanta.conf
  fi
  if [ -n "$RPCPORT" ]; then
    echo "rpcport=${RPCPORT}" >> /sentinel/cosanta.conf
  fi
fi
...
if [ ! -f /sentinel/sentinel.conf ]; then
  ...
  echo "dash_conf=/sentinel/cosanta.conf" > /sentinel/sentinel.conf
  if [ -n "$RPCHOST" ]; then
